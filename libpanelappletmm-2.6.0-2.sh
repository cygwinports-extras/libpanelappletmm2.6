...
autotool() {
	...
	/usr/bin/aclocal -I scripts || exit 1
	/usr/bin/libtoolize --force || exit 1
	/usr/bin/autoconf --force || exit 1
	/usr/bin/automake -a -c -f || exit 1
	cd ${T}
	if [ -f ${P}/INSTALL ] ; then
...
mkpatch() {
	...
	diff -urN -x '.build' -x '.inst' -x '.sinst' -x 'configure' \
		...
		-x 'intltool*' -x 'autom4te.cache' -x 'reference' \
